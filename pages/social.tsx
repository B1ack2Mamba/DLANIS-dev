...
const Card = ({
  ...
  <div className={`rounded-2xl border border-neutral-800 bg-neutral-900 text-neutral-100 shadow-sm ${className}`}>
    {children}
  </div>
);
const CardHeader = ({
...
export default function SocialPage() {
  ...
  const [myThreads, setMyThreads] = useState<{ pk: PublicKey; a: string; b: string; msgCount: number }[]>([]);

  // ====== Каталог профилей / Общая лента ======================================
  const [allProfiles, setAllProfiles] = useState<{ pk: PublicKey; owner: string; name: string; level: number; bio: string }[]>([]);
  const [profilesQuery, setProfilesQuery] = useState("");
  const [allPosts, setAllPosts] = useState<any[]>([]); // глобальная лента
  ...
  // Жёсткий сбор моих постов (индексный + all)
  ...
  // ====== Мои треды (все диалоги) =============================================
  const pullMyThreads = useCallback(async () => {
    if (!program || !wallet.publicKey) return;
    try {
      const acc = program.account as any;
      const me = wallet.publicKey.toBase58();
      const asA = await acc.dmThread.all([{ memcmp: { offset: 8, bytes: me } }]);        // a == me
      const asB = await acc.dmThread.all([{ memcmp: { offset: 8 + 32, bytes: me } }]);   // b == me
      const merged = [...asA, ...asB];
      const map = new Map<string, { pk: PublicKey; a: string; b: string; msgCount: number }>();
      for (const it of merged) {
        const a = new PublicKey(it.account.a).toBase58();
        const b = new PublicKey(it.account.b).toBase58();
        map.set(it.publicKey.toBase58(), { pk: it.publicKey, a, b, msgCount: Number(it.account.msgCount ?? 0) });
      }
      const arr = Array.from(map.values());
      arr.sort((x, y) => y.msgCount - x.msgCount);
      setMyThreads(arr);
    } catch (e) {
      console.debug("pullMyThreads error", e);
      setMyThreads([]);
    }
  }, [program, wallet.publicKey]);
    ...
    async (threadKey: PublicKey | null) => {
      ...
      try {
        const list = await acc.dmMessage.all([
          { memcmp: { offset: 8, bytes: threadKey.toBase58() } }, // thread at 8
        ]);
  ...
  // ====== Каталог профилей (все профили) ======================================
  const pullAllProfiles = useCallback(async () => {
    if (!program) return;
    try {
      const acc = program.account as any;
      const list = await acc.profile.all();
      const arr = list.map((it: any) => ({
        pk: it.publicKey,
        owner: new PublicKey(it.account.owner).toBase58?.() ?? String(it.account.owner),
        name: u8ToString(it.account.displayName),
        bio: u8ToString(it.account.bio),
        level: Number(it.account.level ?? 0),
      }));
      arr.sort((a, b) => b.level - a.level || a.owner.localeCompare(b.owner));
      setAllProfiles(arr);
    } catch (e) {
      console.debug("pullAllProfiles error", e);
      setAllProfiles([]);
    }
  }, [program]);

  // ====== Общая лента постов (все посты) ======================================
  const pullAllPosts = useCallback(async () => {
    if (!program) return;
    try {
      const acc = program.account as any;
      ...
      const arr = list.map((it: any) => {
        const p = it.account;
        const author = new PublicKey(p.author).toBase58();
        const created = Number(p.createdTs ?? 0);
        const min = Number(p.minLevel ?? 0);
        const max = Number(p.maxLevel ?? 0);
        const len = Number(p.contentLen ?? 0);
        const bytes: Uint8Array = p.content as Uint8Array;
        const body = u8ToString(bytes, len);
        return {
          _pk: it.publicKey,
          author,
          created,
      ...
      arr.sort((a, b) => b.created - a.created || b.index - a.index);
      setAllPosts(arr);
    } catch (e) {
      console.debug("pullAllPosts error", e);
      setAllPosts([]);
    }
  }, [program]);
  ...
  const pullState = useCallback(async () => {
    if (!program || !wallet.publicKey || !configPda || !profilePda || !extraPda) {
      // но глобальные каталоги можно грузить и без кошелька
    ...
    try {
      ...
      await Promise.all([
        ...
        pullAllProfiles(),
        pullAllPosts(),
        threadPda ? pullThreadMessages(threadPda) : Promise.resolve(),
      ]);
    } finally {
      setRefreshing(false);
    }
  ...
  const onCloseProfile = useCallback(async () => {
    if (!program || !wallet.publicKey || !profilePda) return;
    if (!confirm("Удалить профиль? PDA будет закрыт, а рент вернётся владельцу.")) return;
    setLoading(true);
    try {
      await program.methods
        .closeProfile()
  ...
  }, [program, wallet.publicKey, peer, dmThreadPda, tryFetch, pullThreadMessages, pullMyThreads]);
  ...
  const points = userStats ? Number(userStats.points ?? 0) : 0;

  const profilesFiltered = useMemo(() => {
    const q = profilesQuery.trim().toLowerCase();
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 md:py-8">
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-4">
          {/* Левый столбец (xl: 3 колонки): профиль, лента, каталог */}
          <div className="space-y-6 xl:col-span-3">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDesc>Свежие сверху • показаны все посты из ончейна</CardDesc>
              </CardHeader>
              <CardContent className="space-y-3">
                {allPosts.length === 0 ? (
                  ...
                  allPosts.map((p) => (
                    <div key={p._pk.toBase58()} className="rounded-xl border border-neutral-800 p-4">
                      <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-400">
                        <span className="text-neutral-200">{shortPk(p.author)}</span>
                        <span>•</span>
                        <span>{new Date(p.created * 1000).toLocaleString()}</span>
                        <span>•</span>
                        <span>L{p.min}–L{p.max}</span>
                        <span>•</span>
                        <span>#{p.index}</span>
                      </div>
                ...
                {posts.length === 0 ? (
                  ...
                  posts.map((p) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-400">
                          <span className="text-neutral-200">{shortPk(wallet.publicKey!)}</span>
                          <span>•</span>
                          <span>L{Number(p.minLevel)}–L{Number(p.maxLevel)}</span>
                          <span>•</span>
                          <span>#{Number(p.index)}</span>
                          <span>•</span>
                          <span>{new Date(Number(p.createdTs) * 1000).toLocaleString()}</span>
                        </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDesc>Все профили по ончейну</CardDesc>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex gap-2">
                  <Input
                    placeholder="Поиск по имени/био/адресу…"
                    value={profilesQuery}
                    onChange={(e) => setProfilesQuery(e.target.value)}
                  />
                  <Button variant="ghost" onClick={() => { setProfilesQuery(""); }}>
                    Сброс
                  </Button>
                </div>
                {profilesFiltered.length === 0 ? (
                  <div className="text-sm text-neutral-400">Профилей не найдено.</div>
                ) : (
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    {profilesFiltered.map((p) => (
                      <div key={p.pk.toBase58()} className="rounded-xl border border-neutral-800 p-4">
                        <div className="flex items-center justify-between">
                          <div className="text-sm">
                            <div className="font-semibold">{p.name || "—"}</div>
                            <div className="text-neutral-400">{shortPk(p.owner)} • L{p.level}</div>
                          </div>
                          <div className="flex gap-2">
                            <Button
                              variant="secondary"
                              onClick={() => setPeer(p.owner)}
                            >
                            ...
                            <Button
                              variant="ghost"
                              onClick={() => setFriendTarget(p.owner)}
                            >
          ...
          {/* Правый столбец: GPT / Новый пост / Контакты / DM */}
          <div className="space-y-6 xl:sticky xl:top-6 h-fit">
                          ...
                          return (
                            <div key={r.publicKey.toBase58()} className="flex items-center justify-between gap-2 rounded-xl border border-neutral-800 p-3 text-sm">
                              ...
                              <div className="flex gap-2">
                                <Button
                                  variant="secondary"
                                  onClick={() => setPeer(other.toBase58())}
                                >
                                  Открыть DM
                                </Button>
                              </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDesc>Все треды, где вы участник</CardDesc>
              </CardHeader>
              <CardContent className="space-y-2">
                {myThreads.length === 0 ? (
                  <div className="text-sm text-neutral-400">Тредов пока нет.</div>
                ) : (
                  myThreads.map((t) => {
                    const other = t.a === wallet.publicKey?.toBase58() ? t.b : t.a;
                    return (
                      <div key={t.pk.toBase58()} className="flex items-center justify-between rounded-xl border border-neutral-800 p-3 text-sm">
                        <div className="text-neutral-300">
                          {shortPk(other)} <span className="text-neutral-500">•</span> msgs: {t.msgCount}
                        </div>
                        <Button
                          variant="secondary"
                          onClick={async () => {
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {threadPda ? (
                  ...
                  <div className="text-xs text-neutral-400">Выберите пользователя (в каталоге или списке друзей) или укажите адрес и нажмите «Открыть», чтобы загрузить/создать диалог.</div>
                )}
              </CardContent>
            </Card>
          </div>
