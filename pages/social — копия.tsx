...
export default function SocialPage() {
  ...
  // Формы (редактирование прячем под кнопку)
  ...
  }, [program, wallet.publicKey, profilePda, displayName, bio, pullState]);
  ...
  const onSendDm = useCallback(async () => {
    ...
    try {
      ...
      // Если видим «already been processed», считаем, что транза прошла и просто перечитываем
      const msg = String(e?.message || e || "");
      if (msg.toLowerCase().includes("already been processed")) {
        await pullThreadMessages(threadPda);
        setDmBody("");
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-400">
                          ...
                          <span>
                            ts: <span className="text-neutral-2 00">{Number(p.createdTs)}</span>
                          </span>
                        </div>
